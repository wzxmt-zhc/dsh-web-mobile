...
function findHeader(headers, id) {
                ...
                if (live !== undefined && sessions !== undefined) {
                    ...
                    try {
                        ...
                        if (agent !== undefined) {
                            ...
                            await withTimeout(agent.whenIdle(), IDLE_TIMEOUT_MS, `agent for session '${sessionId}' did not converge to idle within ${IDLE_TIMEOUT_MS}ms`);
                        }
                        ...
                        const agentEntry = agentRegistry?.store?.get(sessionId);
                        if (agentEntry !== undefined)
                            agentRegistry?.detachEntered?.(agentEntry);
                        const sessionStore = sessions;
                        sessionStore?.store?.get(sessionId)?.detach?.();
                    }
                    catch (error) {
